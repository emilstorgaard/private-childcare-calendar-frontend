...
<div class="min-h-screen flex flex-col">
    <header class="relative sticky top-0 z-40 bg-brand-600 text-white shadow-md">
        {#if $navigating}
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6">
            <div class="flex items-center justify-between h-20">
                <a href="/" class="group flex items-center shrink-0 my-2">
                    <div
                        class="bg-white p-1.5 rounded-2xl shadow-md border border-brand-500/10 transition-all duration-300 group-hover:shadow-lg group-hover:scale-102"
                    >
                        <img
        ...
        {#if navOpen}
            <nav class="absolute top-full left-0 w-full lg:hidden border-t border-white/15 bg-brand-600 px-4 py-3 space-y-1 shadow-xl z-50">
                {#each navItems as item}
                    <a
                        href={item.href}
                        onclick={() => (navOpen = false)}
                        class="flex items-center gap-3 px-3 py-2.5 rounded-lg font-medium transition-colors {isActive(
                            item.href
                        )
                            ? 'bg-white/20'
                            : 'hover:bg-white/10'}"
    ...
    {#if navOpen}
        <div 
            class="fixed inset-0 top-20 bg-black/20 backdrop-blur-sm z-30 lg:hidden transition-all duration-200"
            onclick={() => (navOpen = false)}
        ></div>
    {/if}
